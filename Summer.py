...
def assign_course_maximums(maximums, course_names):  # create a dictionary where key = course and value = max
...
for repetition in range(0, sample_size):
    ...
    maximums = df.iloc[-1, 1:].replace({"none": "16"}, regex=True)  # bottom row (-name column) (series --> dict)
    ...
    for key, value in data.items():
        ...
        try:
            ...
            value.append("")
#    if not rank_flags:  # Check if there are any flags, add if not
...
#          "Exiting programme...")
out_df.to_csv(r"C:\Users\JcesW\Desktop\SUMMER GIFT4.csv", index=False)
print("CSV Created")
